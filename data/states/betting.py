...
class Betting(tools._State):
    ...
    def __init__(self):
        ...
        self.labels = []
        self.title = GLabel(self.labels, self.font, 48, "Mistletoe Downs", "darkgreen",
                                   {"midtop": (screen_rect.left + 200, 10)}, "white")
        self.race_title = GLabel(self.labels, self.font, 32,
                                           self.distance_map[self.distance],
                                           "gray1", {"midtop": (self.title.rect.centerx, 
                                           self.title.rect.bottom + 10)}, "white") 
        self.header1 = GLabel(self.labels, self.font, 24, "# Name", "darkgreen",
                                         {"topleft": (35, self.title.rect.bottom + 50)}, "white")
        self.header2 = GLabel(self.labels, self.font, 24, "Odds", "darkgreen", {"topleft":
                                         (210, self.title.rect.bottom + 50)}, "white")
        self.header3 = GLabel(self.labels, self.font, 24, "Speed   Stamina", "darkgreen",
                                         {"topleft": (280, self.title.rect.bottom + 50)}, "white")
        ...
        for racer in self.racers:
            ...
            racer.name_label = Label(self.font, 18, "{0:5})  {1}".format(racer.num,
                                                    racer.name), "gray1", {"topleft": (5,
                                                    top)})
            racer.odds_label =  Label(self.font, 18, racer.odds_text, "gray1", {"topleft":
                                                  (230, top)}, "white")           
            racer.stats_label = Label(self.font, 18, "{}{:18}".format(racer.speed,
        ...
        self.wager_buttons = []
        self.cur_title = GLabel(self.labels, self.font, 18, "Bet Type: ", "gray1", 
                                         {"topleft": (20, new_top + 10)}, "white")
        
        horiz = 250
        
        for wt in self.wager_types:
            wt_label = Label(self.font, 14, wt, "gray1", {"topleft": (0, 0)}, "white")
            button = PayloadButton(horiz, new_top, 60, 40, wt_label, wt)
            self.wager_buttons.append(button)
            horiz += 70
        new_top += button.rect.height + 20
        self.amt_title = GLabel(self.labels, self.font, 20, "Bet Amount: ", "gray1",
                                     {"topleft": (20, new_top + 20)}, "white")
        self.amount_buttons = []
        horiz = 250
        for num in [1, 5, 10, 25, 100, 500, 1000]:
            pos_label = Label(self.font, 14, "+{}".format(num), "gray1", {"topleft":
                                         (0, 0)}, "white")
            pos_button = PayloadButton(horiz, new_top, 50, 30, pos_label,
                                                       num)
            neg_label = Label(self.font, 14, "-{}".format(num), "gray1", {"topleft":
                                         (0, 0)}, "white")
            ...
            horiz += 60
        new_top += 90    
        self.picks_title = GLabel(self.labels, self.font, 20, "Picks: ", "gray1",
                                     {"topleft": (20, new_top + 20)}, "white")
        horiz = 250
        self.pick_buttons = []
        i = 0
        for reindeer in self.racers:
            p_label = Label(self.font, 18, "{}".format(reindeer.num), "gray1",
                                      {"topleft": (0, 0)}, "white")
        ...
        r_label = Label(self.font, 16, "Remove", "gray1", {"topleft": (0, 0)}, "white")
        self.remove_button = Button(horiz, new_top, 70, 30, r_label)
        place_label = Label(self.font, 20, "Place Bet", "darkgreen", {"topleft": (0, 0)},
                                       "white")
        self.place_button = Button(50, self.screen_rect.bottom - 60, 120, 50,
                                                place_label)
        start_label = Label(self.font, 48, "Start Race", "darkgreen", {"topleft": (0, 0)},
                                      "white")
        self.start_button = Button(self.middle_rect.centerx - 125,
        self.screen_rect.bottom - 140, 250, 120, start_label)
        self.cash_title = GLabel(self.labels, self.font, 24, "Cash: ", "gray1", {"topleft": 
                                           (self.middle_rect.left + 10, self.middle_rect.top + 10)},
                                           "white")
        self.total_title = GLabel(self.labels, self.font, 24, "Total Wagers: ", "gray1",
    ...
    def update(self, surface, keys):
        ...
        self.amount_label = GLabel(self.dynamics, self.font, 16,
                                                 "${:.2f}".format(self.current_amount),
                                                 "gray1", {"midleft": (
                                                 self.amt_title.rect.right + 5,
                                                 self.amt_title.rect.centery)}, "white")
        self.picks_label = GLabel(self.dynamics, self.font, 16, "{}".format("-".join(
        ...
        for player_bet in self.player.bets:
            pb_label = GLabel(self.dynamics, self.font, 18, player_bet.text, "gray1",
                                        {"topleft": (left, top)}, "white")
            top += 20
        cash_label = GLabel(self.dynamics, self.font, 18, "${:.2f}".format(self.player.cash), "gray1",
        ...
        bets_label = GLabel(self.dynamics, self.font, 18, "${:.2f}".format(total_bets), "gray1",
