...
class Decoration(object):
    ...
    def update(self, world):
        for elf in [x for x in world.elves if x.state in {"Travelling", "Hauling"}]:
...
class XmasTree(Decoration):
    def __init__(self, index, world):                
        tile_map = ["XX",
                          "TX"]
        super(XmasTree, self).__init__("Xmas Tree", index, world, (32, 32), tile_map)     
        self.image = prepare.GFX["xmastree"]
        self.light_cycle = it.cycle([prepare.GFX["treelights" + str(x)] for x in range(1, 5)])         
        self.lights = next(self.light_cycle)
        self.lights_rect = self.lights.get_rect(topleft=(self.rect.left, self.rect.top + 9))
    ...
    def update(self, world):
        if not world.ticks % 500:
            ...
            for point in points:
                ...
                center_point = (self.rect.left + point[0], self.rect.top + point[1])
                ...
                self.fireworks.append(Firework(color, center_point, x_velocity, y_velocity,
                                                              altitude, delay, self))
...
class Firework(object):
    def __init__(self, color, center_point, x_velocity, y_velocity, altitude, delay, pyro_box):
        ...
        if random.choice((True, False, False)):
            self.images = iter([prepare.GFX[self.color + "firework_ring" + str(i)] for i in range(1, 15)])
        else:
            self.images = iter([prepare.GFX[self.color + "firework" + str(i)] for i in range(1, 15)])        
